...
export const ColladaLoader = function(a) {
    this.manager = void 0 !== a ? a : THREE.DefaultLoadingManager
};


export function load (a, b, c, d) {
    function e(a) {
        var b = a.split("/");
...
function parse(a, b) {
    ...
    function E(a) {
        function e(a) {
            ...
            if (void 0 !== c) {
                ...
                if (void 0 !== f && void 0 !== f.technique) {
                    ...
                    e.offset.set(g.offsetU, g.offsetV);
                    e.repeat.set(g.repeatU, g.repeatV);
                } else 
        ...
        for (var g in f) {
            ...
            switch (g) {
                case "diffuse":
                    h.color && d.color.fromArray(h.color);
                    h.texture && (d.map = e(h.texture));
                    break;
                case "specular":
    ...
    console.time("ColladaLoader: Parse");
     j(ja, "library_images", "image", n); 
     j(ja, "library_effects", "effect", q); 
     j(ja, "library_materials", "material", D); 
     j(ja, "library_cameras", "camera", G); 
     j(ja, "library_lights", "light", M);
      j(ja, "library_geometries", "geometry", R); j(ja, "library_nodes", "node", aa); j(ja, "library_visual_scenes", "visual_scene", ea); console.timeEnd("ColladaLoader: Parse"); console.time("ColladaLoader: Build"); k(ma.images, o); k(ma.effects, B); k(ma.materials, E); k(ma.cameras, K); k(ma.lights, P); k(ma.geometries, X); k(ma.nodes, ca); k(ma.visualScenes, fa); console.timeEnd("ColladaLoader: Build");
