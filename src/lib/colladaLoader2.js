...
THREE.ColladaLoader.prototype = {
    ...
    parse: function (a, b) {
        ...
        function o(a) {
            ...
            var c = a.init_from;
            return 0 !== b && (c = b + c) ? m.load(c) : null;
        }
        ...
        function Y(a, b, c) {
            ...
            var d = a.p,
                ...
                k = [];
            if (void 0 !== a.vcount) {
                for (let l = 0, m = 0, n = f.length; m < n; m++) {
                    var o = f[m];
                    if (4 === o) {
                        let p = l + 0 * e,
                            q = l + 1 * e,
        ...
        function aa(a) {
            for (var b = {
                ...
                if (1 === e.nodeType) switch (e.nodeName) {
                    ...
                    case "instance_node":
                        ...
                        break;
                    case "matrix":
                        var g0 = d(e.textContent);
                        b.matrix.multiply($.fromArray(g0).transpose());
                        break;
                    case "translate":
                        var g2 = d(e.textContent);
                        _.fromArray(g2); b.matrix.multiply($.makeTranslation(_.x, _.y, _.z));
                        break;
                    case "rotate":
                        var g1 = d(e.textContent),
                            h = THREE.Math.degToRad(g1[3]);
                        b.matrix.multiply($.makeRotationAxis(_.fromArray(g1), h));
                        break;
                    case "scale":
            ...
            return null !== a.getAttribute("id") && (ma.nodes[a.getAttribute("id")] = b)? b: null;
        }
        ...
        function ca(a) {
            ...
            for (let i = 0, j = e.length; i < j; i++) b.push(L(e[i]).clone());
            for (let i = 0, j = f.length; i < j; i++) b.push(Q(f[i]).clone());
            ...
            for (let i = 0, j = h.length; i < j; i++) b.push(da(h[i]).clone());
             n = null;
            if (0 === d.length && 1 === b.length) n = b[0];
            else {
                n = new THREE.Group();
                for (let i = 0; i < b.length; i++) n.add(b[i])
            }
